...
const ExamsListPage = async ({
  ...
  const lessonCondition: Prisma.LessonWhereInput = {};
  ...
  if (queryParams) {
    for (const [key, value] of Object.entries(queryParams)) {
      if (value !== undefined) {
        switch (key) {
          case "classId":
            lessonCondition.classId = parseInt(value);
            break;
          case "teacherId":
            lessonCondition.teacherId = value;
            break;
          case "search":
            lessonCondition.subject = {
  ...
  // 2. تطبيق شروط الصلاحيات بدقة
  switch (role) {
    case "admin":
      break;
    case "teacher":
      lessonCondition.teacherId = currentUserId!;
      break;
    case "student":
      lessonCondition.class = {
  ...
  if (Object.keys(lessonCondition).length > 0) {
    query.lessons = lessonCondition;
  }
