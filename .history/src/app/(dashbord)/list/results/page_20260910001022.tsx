...
const ResultsListPage = async ({
  ...
  const data = dataRes.map((item) => {
    ...
    const assessment = item.exam || item.assessment;    if (!assessment) return null;            id: item.id,      title: assessment.title,      studentName: item.studnet.name,      studentSurname: item.studnet.name,    };  });        return (    <section className="bg-white p-4 rounded-md flex-1 m-4 mt-0 ">      {/* TOP */}      <div className="flex items-center justify-between">        <h1 className=" hidden md:block text-lg font-semibold">All Results</h1>        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          <TableSerach />
