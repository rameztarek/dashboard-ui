...
const rounderRow = (items: LessonList) => (
  <tr
    key={items.id}
  ...
  >
    <td className="flex items-center gap-4 p-4">
      <div className="flex flex-col">
        <h3 className="font-semibold">{items.subject.name}</h3>
      </div>
    </td>
    <td>{items.class.name}</td>
    <td className="hidden md:table-cell">{items.teacher.name + " " + items.teacher.surname}</td>
    <td>
      <div className="flex items-center gap-2">
        {role === "admin" && (
          <>
            <FormModle table="lessons" type="update" data={items} />
            <FormModle table="lessons" type="delete" id={items.id} />
          </>
        )}
      </div>
    </td>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex gap-4 items-center self-end">
            ...
            {role === "admin" && <FormModle table="lessons" type="create" />}
          </div>
        </div>
      </div>
      {/* LIST */}
