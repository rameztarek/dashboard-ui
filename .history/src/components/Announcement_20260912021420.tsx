...
const Announcement = async () => {
  ...
  const {userId , ssesionsClaims} = auth();
  const role = (ssesionsClaims.metaData as {role?: string})?.role;

  const roleCondintion = {
    admin:{},
    teacher:{lessons:{some:{teacherId:userId}}},
    syudent:{students:{some:{id:userId}}},
    teacher:{lessons:{some:{parentId:userId}}},
  }

  const Announcement = await prisma.announcement.findMany({
    where:{
      OR:[
        {classId:null  },{class:roleCondintion[role]}
      ]
    }
  })
