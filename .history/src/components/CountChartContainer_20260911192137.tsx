...
const CountChartContainer = async () => {
  ...
  // قمنا بتعريف نوع المتغير d بوضوح ليطابق ما يرجعه Prisma
  const boys = data.find((d: { sex: string; _count: { _all: number } }) => d.sex === "MALE")?._count._all || 0;
  const girls = data.find((d: { sex: string; _count: { _all: number } }) => d.sex === "FEMALE")?._count._all || 0;
